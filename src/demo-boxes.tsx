...
const MeshGroupFunctionComponent = toFunctionComponent<[Function?], MeshView>(class extends Component<[Function?], MeshView> {
    parent: MeshView;
    onInit(parent: MeshView) {
        this.parent = parent;
    }
    componentWillMount(data: Function) {
...
const Box = toFunctionComponent(class extends Component<Props, MeshView> {
    ...
    componentWillMount(data: Props[0], config: Props[1]) {
    ...
    componentWillUpdate(data: Props[0], config: Props[1]) {
        if (needDraw) {
    ...
    render(data: Props[0], config: Props[1], child: Props[2]) {
        if (child) {
            child();
        }
    }
